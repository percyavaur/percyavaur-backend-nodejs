...
export default async function validate_session(req, res) {
  ...
  function _validateCookie() {
    if (!refreshToken || refreshToken.length <= 0) {
      removeCookies(res);
      res.status(400).send({
        error_code: "ERROR_VALIDATE_SESSION_0002",
    ...
    jsonwebtoken.verify(refreshToken, jwtConfig.key, async function (
      ...
      if (err) {
        ...
        if (err.name == "TokenExpiredError") {
          res.status(400).send({
            error_code: "ERROR_VALIDATE_SESSION_0003",
          ...
          res.status(400).send({
            error_code: "ERROR_VALIDATE_SESSION_0004",
          ...
          res.status(400).send({
            error_code: "ERROR_VALIDATE_SESSION_0005",
        ...
        if (decoded.tokenType != "refreshToken") {
          removeCookies(res);
          res.status(400).send({
            error_code: "ERROR_VALIDATE_SESSION_0006",
            message: "Need refresh token.",
            destroy: true,
            success: false,
          });
          ...
          if (data_found.error) {
            ...
            res.status(500).send({
              error_code: "ERROR_VALIDATE_SESSION_0007",
            ...
            if (expires < moment()) {
              removeCookies(res);
              res.status(409).send({
                error_code: "ERROR_VALIDATE_SESSION_0008",
              ...
              if (user_found.error) {
                ...
                if (id_status != 1) {
                  ...
                  removeCookies(res);
                  res.status(409).send({
                    error_code: "ERROR_VALIDATE_SESSION_0010",
                  ...
                  const timeDifference = moment(expires).diff(moment(), "days");
                  var newExpires =
                    timeDifference <= 7
                      ? moment().add(15, "days").format()
  ...
  try {
    ...
    console.error(error);
    res.status(500).send({
      error_code: "ERROR_VALIDATE_SESSION_0001",
      message: "Some error occurred while validating session.",
      success: false,
    });
  }
